...
1. **학교 알리미 에이전트를 확장해봅시다.**  
   우리 학교, 학생, 학부모에게 실제로 도움이 되는 질문을 더 정의해보고  
   공시 데이터를 기반으로 한 새로운 에이전트 시나리오를 고민해봅시다.

2. **다른 공공 API들도 연결해봅시다.**  
   학교 현장에서 활용도가 높은 API 예시는 다음과 같습니다.
